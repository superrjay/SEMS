...
if(Auth::isLoggedIn()) redirect('/clinic/dashboard.php');
$errors=[];
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if(empty($errors)){try{$db=Database::getConnection();
        ...
        if($u&&password_verify($password,$u['password_hash'])){Auth::login((int)$u['user_id'],$u['role_name'],['first_name'=>$u['first_name'],'last_name'=>$u['last_name'],'email'=>$u['email']]);
            $db->prepare("UPDATE users SET last_login=NOW() WHERE user_id=?")->execute([$u['user_id']]);log_audit($db,'System','Login','user',(int)$u['user_id']);flash('success','Welcome back, '.$u['first_name'].'!');redirect('/clinic/dashboard.php');}
        else{$errors[]="Invalid email or password.";}
    }catch(\PDOException $e){$errors[]="Login unavailable.";}}
}
?><!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>Login — Clinic+</title><link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/uikit@3.21.6/dist/css/uikit.min.css">
...
.logo{font-size:2.2rem;font-weight:800;color:#0f766e;text-align:center;letter-spacing:-.5px}.logo b{color:#14b8a6}
</style></head><body>
<div class="lc"><div class="lc-inner">
<div class="logo">Clinic<b>+</b></div>
<p class="uk-text-center uk-text-muted" style="font-size:.82rem;margin-top:4px">Medical Services System</p>
...
<div class="uk-margin"><label class="uk-form-label" style="font-size:.78rem;font-weight:600;color:#555">Email</label><input class="uk-input" type="email" name="email" value="<?=sanitize_output($_POST['email']??'')?>" placeholder="admin@clinic.edu" required style="border-radius:8px"></div>
...
<button type="submit" class="uk-button uk-width-1-1 uk-margin-small-top" style="border-radius:8px;background:#0f766e;color:#fff;font-weight:600;height:44px">Sign In</button>
</form>
<p class="uk-text-center uk-text-muted uk-margin-small-top" style="font-size:.72rem">Run <a href="/clinic/setup_admin.php" style="color:#14b8a6">setup_admin.php</a> first &middot; admin@clinic.edu / admin123</p>
</div></div>
